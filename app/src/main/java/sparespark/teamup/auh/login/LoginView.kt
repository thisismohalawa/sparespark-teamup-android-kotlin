...
import sparespark.teamup.auh.login.buildlogic.LoginInjector
...
private const val TEST_EMAIL = "test@teamup.com"
private const val TEST_PASSWORD = "000000"
...
class LoginView : BaseAuthView(), View.OnClickListener,
    ...
    private fun setupViewModel() {
        ...
        viewModel.handleEvent(AuthEvent.GetTestUserCredential)
    }
    ...
    private fun LoginViewModel.startObserving() {
        ...
        testCredentialAttempt.observe(viewLifecycleOwner) {
            binding?.apply {
                itemEmail.edText.setText(TEST_EMAIL)
                itemPassword.edText.setText(TEST_PASSWORD)
            }
        }
    ...
    override fun onClick(view: View?) {
        when (view?.id) {
            R.id.btn_signup -> if (findNavController().currentDestination?.id == R.id.loginView)
                findNavController().navigate(LoginViewDirections.navigateToSignupView()) else Unit
